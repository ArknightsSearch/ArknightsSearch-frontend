...
function jump (target) {
    if (target === 'local') {
        ...
        goto(StoryJumpUrl.astr + storyInfo.value.id)
    } else if (target === 'prts') {
        const g = storyInfo.value.id.split('/')
        const ng = g[g.length - 1].split('_')
        if (storyInfo.value.type === 'Memory') {
            if (ng[ng.length - 1] !== '1') {
                goto(StoryJumpUrl.prts + storyInfo.value.zone + '/干员密录/' + ng[ng.length - 2] + '-' + ng[ng.length - 1])
            } else {
                api.post({
                    ...
                    success (resp) {
                        if (resp.data) {
                            goto(StoryJumpUrl.prts + storyInfo.value.zone + '/干员密录/' + ng[ng.length - 2] + '-1')
                        } else {
                            goto(StoryJumpUrl.prts + storyInfo.value.zone + '/干员密录/' + ng[ng.length - 2])
                        }
                    }
                })
            }
            ...
            if (ng.indexOf('st') !== -1 || ng.indexOf('spst') !== -1) {
                ...
                goto(StoryJumpUrl.prts + n2g.slice(0, n2g.length - 1).join('_') + '/BEG')
            } else if (ng.indexOf('end') !== -1) {
                goto(StoryJumpUrl.prts + n2g.slice(0, n2g.length - 1).join('_') + '/END')
            } else {
                for (const i in ng) {
